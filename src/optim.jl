...
function optimauglag(m, reg, w0 = uniformweights(length(m.xs)); optimizer=:LD_MMA, maxeval=20)
...
function optimineq(m, reg, w0=uniformweights(length(m.xs)); optimizer=:LD_MMA)
